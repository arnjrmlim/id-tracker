...
{{-- Flash messages (e.g. redirect-back errors from ExportController) --}}
@if(session('error'))
...
{{-- Tip: table-based selection export --}}
<div class="alert alert-info d-flex align-items-start gap-2 mb-4" role="note">
    <i class="bi bi-info-circle-fill flex-shrink-0 mt-1"></i>
    <div>
        <strong>Tip — Export selected rows:</strong>
        To export only specific records, go to the
        <a href="{{ route('id-records.index') }}" class="alert-link">ID Records table</a>,
        select the rows you want using the checkboxes, then click
        <strong>Export</strong> directly from the toolbar.
        The forms below export <em>all</em> records that match the chosen filter.
    </div>
</div>
